...
Method:
  1. Forward hook on model.encoder.blocks[-1] → activations [B, N+1, D]
  2. Backward hook on same layer → gradients [B, N+1, D]
  3. Weight activations by mean gradient over feature dim: alpha = mean(grad, dim=-1)
...
sys.path.insert(0, str(Path(__file__).parent.parent / 'tangerine' / '3D-MAE-MedImaging'))
...
class GradCAMHooks:
    """
    Attaches forward + backward hooks to the final transformer block.
    Stores activations [B, N+1, D] and gradients [B, N+1, D].
    """
    def __init__(self, model):
        self.activations = None
        self.gradients   = None
        target_layer     = model.encoder.blocks[-1]   # block 23
...
def compute_gradcam(model, volume_tensor, target_year=None):
    ...
    activations = hooks.activations  # [1, N+1, D]
    gradients   = hooks.gradients    # [1, N+1, D]
    hooks.remove()
    ...
    act  = activations[:, 1:, :].detach()   # [1, N, D]
    grad = gradients[:, 1:, :].detach()     # [1, N, D]
