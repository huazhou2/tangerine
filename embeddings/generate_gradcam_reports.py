...
def build_report(patient_id, pred_probs_cal, pred_probs_raw, meta, gradcam_dir, images_dir, out_pdf):
    ...
    # Format per-year probability table
    ...
    body = (
        ...
        'to predict the 6-year lung cancer risk for the following case:\n\n'
        ...
        "Predicted cumulative cancer probability by year (calibrated):\n"
        f'  {yr_lines_cal}\n\n'
        "Predicted cumulative cancer probability by year (raw):\n"
        f'  {yr_lines_raw}\n\n'
        'Follow-up:\n'
        f'{cancer_text}\n\n'
        'The Grad-CAM heatmap highlights regions that most strongly activated '
        'the cancer prediction (red = high contribution, blue = low contribution):'
    )
    ...
    pdf.cell(w=210, h=22,
             txt=f'TANGERINE 6yr Grad-CAM Report - CT {patient_id}',
    ...
    if top3:
        top3_lines = '\n'.join(
            ...
            f'   (mean Grad-CAM: {score:.4f})'
            for i, (s_idx, score) in enumerate(top3)
        )
        pdf.multi_cell(w=200, h=8,
                       txt='\nGrad-CAM peak region (slices to review in PACS):\n'
                           + top3_lines,
    ...
    if grid_img is not None:
        ...
        pdf.cell(w=210, h=10,
                 txt='Grad-CAM - 9 representative axial slices'
                     ' (lung window CT + Grad-CAM overlay)',
                 ln=1, align='C')
        pdf.set_font('Arial', '', 9)
        pdf.cell(w=210, h=6,
                 txt='Red label = slice index.  Yellow label = mean Grad-CAM intensity on slice.',
                 ln=1, align='C')
        ...
        pdf.cell(w=210, h=20,
                 txt='Grad-CAM map not available for this patient.',
                 ln=1, align='C')
