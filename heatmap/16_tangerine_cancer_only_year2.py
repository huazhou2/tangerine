"""
TANGERINE Cancer-Only Longitudinal Heatmap — Year 2 Prediction
===============================================================
Visualizes cancer patients' risk trajectories using Year-2 predictions.
Similar to Year 1 but focused on 2-year cancer risk.

Input:
  - ../dataset_splits/test.csv: Patient metadata
  - ../results_20260605/test_predictions.csv: Model predictions (pred_2)
  - ../scan_master_with_lrads_value_v3_with_base.csv: LRADS scores

Output:
  - ../results_20260605/heatmaps/tangerine_cancer_only_year2.png

Features:
  - Cell color: Year-2 cancer risk prediction (Red=high, Green=low)
  - LRADS: Overlaid as text (no background)
  - PURPLE *: Diagnosis month (centered)
  - Y-axis: Cancer patients (red text, sorted)
"""
...
# Filter to patients with >1 scan
...
# Filter to CANCER PATIENTS ONLY
...
# Convert scan dates to year-month periods
data_cancer['year_month'] = data_cancer['ct_date'].dt.to_period('M')
data_cancer['pred'] = data_cancer['pred_2_pred']  # Year 2 prediction
...
print("="*80)
print("TANGERINE CANCER-ONLY LONGITUDINAL HEATMAP — YEAR 2")
print("="*80)
print(f"\nTotal cancer patients: {len(cancer_patients)}")
...
def create_heatmap_data(patient_ids, all_months_list, data_source):
    """Create heatmap arrays with predictions and LRADS"""
    heatmap_pred = []
    heatmap_lrads = []
    ...
    for pat_id in patient_ids:
        ...
        # Build month → prediction and month → LRADS mappings
        pred_dict = {}
        lrads_dict = {}
...
# Overlay LRADS text and diagnosis markers
for i in range(len(cancer_patients)):
    for j in range(len(all_months)):
        # LRADS score (if available)
        ...
        # Diagnosis marker (purple asterisk, centered in cell)
...
# Y-axis: Full patient IDs (no [T] marker)
ax.set_yticks(np.arange(len(cancer_patients)))
y_labels = [str(p) for p in cancer_patients]
ax.set_yticklabels(y_labels, fontsize=16, color='red', fontweight='bold')
...
title = f'TANGERINE Cancer-Only Predictions (Year 2): n={len(cancer_patients)} patients\n'
title += f'Cell color = Year-2 cancer probability | PURPLE * = Diagnosis month'
...
cbar = plt.colorbar(im, ax=ax, label='Year-2 Cancer Probability')

plt.tight_layout()
heatmap_dir = os.path.join(parent_dir, 'results_20260605/heatmaps')
os.makedirs(heatmap_dir, exist_ok=True)
plt.savefig(os.path.join(heatmap_dir, 'tangerine_cancer_only_year2.pdf'), dpi=300, bbox_inches='tight')
print(f"\n✓ Saved: tangerine_cancer_only_year2.pdf")
plt.close()

print(f"\n✅ TANGERINE Year 2 heatmap complete!")
