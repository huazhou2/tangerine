"""TANGERINE All Patients Year 2 (Cancer + Non-Cancer)"""
import pandas as pd
import numpy as np
...
data_multi['pred'] = data_multi['pred_2_pred']
...
print("="*80)
print("TANGERINE ALL PATIENTS YEAR 2 (CANCER + NON-CANCER)")
print("="*80)
...
title = f'TANGERINE All Patients Year 2: n={len(all_patients)} (Cancer={len(cancer_patients)}, Non-Cancer={len(non_cancer_patients)})\n'
...
cbar = plt.colorbar(im, ax=ax, label=f'Year-2 Cancer Probability')

plt.tight_layout()
heatmap_dir = os.path.join(parent_dir, 'results_20260605/heatmaps')
os.makedirs(heatmap_dir, exist_ok=True)
plt.savefig(os.path.join(heatmap_dir, 'tangerine_all_patients_year2.pdf'), dpi=150, bbox_inches='tight')
print(f"\n✓ Saved: tangerine_all_patients_year2.pdf")
plt.close()

print(f"✅ Year 2 complete!")
