"""TANGERINE Cancer-Only Year 6 Prediction"""
import pandas as pd
import numpy as np
...
data_cancer['pred'] = data_cancer['pred_6_pred']
...
print("="*80)
print("TANGERINE CANCER-ONLY LONGITUDINAL HEATMAP — YEAR 6")
print("="*80)
...
title = f'TANGERINE Cancer-Only Predictions (Year 6): n={len(cancer_patients)} patients\n'
...
cbar = plt.colorbar(im, ax=ax, label='Year-6 Cancer Probability')

plt.tight_layout()
heatmap_dir = os.path.join(parent_dir, 'results_20260605/heatmaps')
os.makedirs(heatmap_dir, exist_ok=True)
plt.savefig(os.path.join(heatmap_dir, 'tangerine_cancer_only_year6.pdf'), dpi=300, bbox_inches='tight')
print(f"✓ Saved: tangerine_cancer_only_year6.pdf")
plt.close()
