"""TANGERINE All Patients Year 3 (Cancer + Non-Cancer)"""
import pandas as pd
import numpy as np
...
data_multi['pred'] = data_multi['pred_3_pred']
...
print("="*80)
print("TANGERINE ALL PATIENTS YEAR 3 (CANCER + NON-CANCER)")
print("="*80)
...
title = f'TANGERINE All Patients Year 3: n={len(all_patients)} (Cancer={len(cancer_patients)}, Non-Cancer={len(non_cancer_patients)})\n'
...
cbar = plt.colorbar(im, ax=ax, label=f'Year-3 Cancer Probability')

plt.tight_layout()
heatmap_dir = os.path.join(parent_dir, 'results_20260605/heatmaps')
os.makedirs(heatmap_dir, exist_ok=True)
plt.savefig(os.path.join(heatmap_dir, 'tangerine_all_patients_year3.pdf'), dpi=150, bbox_inches='tight')
print(f"\n✓ Saved: tangerine_all_patients_year3.pdf")
plt.close()
