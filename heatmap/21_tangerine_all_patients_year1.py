"""TANGERINE All Patients Year 1 (Cancer + Non-Cancer)"""
import pandas as pd
import numpy as np
...
data_multi['pred'] = data_multi['pred_1_pred']
...
print("="*80)
print("TANGERINE ALL PATIENTS YEAR 1 (CANCER + NON-CANCER)")
print("="*80)
...
title = f'TANGERINE All Patients Year 1: n={len(all_patients)} (Cancer={len(cancer_patients)}, Non-Cancer={len(non_cancer_patients)})\n'
...
plt.savefig(os.path.join(heatmap_dir, 'tangerine_all_patients_year1.pdf'), dpi=150, bbox_inches='tight')
print(f"\n✓ Saved: tangerine_all_patients_year1.pdf")
plt.close()
