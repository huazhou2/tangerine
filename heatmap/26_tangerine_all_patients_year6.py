"""TANGERINE All Patients Year 6 (Cancer + Non-Cancer)"""
import pandas as pd
import numpy as np
...
data_multi['pred'] = data_multi['pred_6_pred']
...
print("="*80)
print("TANGERINE ALL PATIENTS YEAR 6 (CANCER + NON-CANCER)")
print("="*80)
...
title = f'TANGERINE All Patients Year 6: n={len(all_patients)} (Cancer={len(cancer_patients)}, Non-Cancer={len(non_cancer_patients)})\n'
title += f'Cell color = Year-6 cancer probability | PURPLE * = Diagnosis month'
...
cbar = plt.colorbar(im, ax=ax, label=f'Year-6 Cancer Probability')

plt.tight_layout()
heatmap_dir = os.path.join(parent_dir, 'results_20260605/heatmaps')
os.makedirs(heatmap_dir, exist_ok=True)
plt.savefig(os.path.join(heatmap_dir, 'tangerine_all_patients_year6.pdf'), dpi=150, bbox_inches='tight')
print(f"\n✓ Saved: tangerine_all_patients_year6.pdf")
plt.close()

print(f"✅ Year 6 complete!")
