...
Supports:
  - Single train/val/test split (default)
  - 5-fold cross-validation (--fold_idx 0-4)
...
def prepare(args):
    ...
    if args.fold_idx is not None:
        print(f"5-FOLD CROSS-VALIDATION (Fold {args.fold_idx})")
    else:
    ...
    if args.fold_idx is not None:
        ...
        print(f"\nGenerating 5-fold splits (using fold {args.fold_idx} as test)...")
        skf = StratifiedKFold(n_splits=5, shuffle=True, random_state=args.seed)
        folds = list(skf.split(patient_df, patient_df['cancer']))

        train_val_idx, test_idx = folds[args.fold_idx]
        train_val_pats = patient_df.iloc[train_val_idx].reset_index(drop=True)
        test_pats = patient_df.iloc[test_idx].reset_index(drop=True)

        # Split train_val (80% of data) into train (70%) and val (10%)
        # Split ratio: val = 10% / 80% = 12.5% of train_val
        train_pats, val_pats = train_test_split(
            train_val_pats, test_size=0.125,  # 10% / 80% = 0.125
            stratify=train_val_pats['cancer'], random_state=args.seed
        )
    else:
        # Standard single split: 70% train, 15% val, 15% test
    ...
    config = {
        ...
        'split_method': '5-fold-cv' if args.fold_idx is not None else 'patient-level',
        'fold_idx': args.fold_idx,  # None for single split, 0-4 for 5-fold
...
if __name__ == '__main__':
    ...
    p.add_argument('--fold_idx',      type=int,   default=None,
                   help='5-fold index (0-4). If None, use single train/val/test split')
