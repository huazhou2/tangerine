#!/bin/bash
#SBATCH --job-name=tangerine_focal_pr
#SBATCH --output=logs/tangerine_focal_%j.out
#SBATCH --error=logs/tangerine_focal_%j.err
...
echo "=================================================="
echo "TANGERINE - 6-Year Survival with Focal Loss + PR-AUC"
echo "=================================================="
...
SCRIPT_DIR="/gpfs/data/tsirigoslab/home/zhouh05/lung_ct/models/tangerine_6yrs_20260611_full"
...
RUN_DIR="$SCRIPT_DIR/outputs/focal_gamma2_pr_auc_$(date +%Y%m%d_%H%M%S)"
...
# ============================================================================
# Check & Regenerate Dataset Splits if Needed (Auto-fix data leakage)
# ============================================================================

if [ ! -d "$DATASET_DIR" ]; then
    echo "=================================================="
    echo "Dataset splits not found - REGENERATING"
    echo "=================================================="
    echo ""
    ...
    if [ $? -ne 0 ]; then
        echo "ERROR: Failed to regenerate dataset splits"
        exit 1
    fi
...
"
    ...
    if [ $? -ne 0 ]; then
        echo "ERROR: Data leakage verification failed"
        exit 1
    fi
    echo "✓ Splits regenerated and verified"
    echo ""
else
    echo "✓ Using existing dataset_splits"
    echo ""
fi
...
echo "=================================================="
echo "TRAINING WITH FOCAL LOSS + PR-AUC"
echo "=================================================="
echo ""
