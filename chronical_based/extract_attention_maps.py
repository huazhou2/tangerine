...
def summary_plot(ct_vol, attn_vol, patient_id, pred_scores, cancer, output_path,
    ...
    Slices are chosen at the peak attention voxel after lung masking.
    Non-lung regions (background air and bone/spine) are masked from attention.
    """
    # Apply lung region mask per-volume before slice selection
    # Normalize CT to [0,1] for masking
    ct_min, ct_max = ct_vol.min(), ct_vol.max()
    ct_norm_vol = (ct_vol - ct_min) / (ct_max - ct_min + 1e-8)
    # Exclude background air (<5th pct) and bone/spine (>65th pct of CT range)
    lung_mask = (ct_norm_vol > 0.05) & (ct_norm_vol < 0.65)
    attn_masked = attn_vol * lung_mask

    # Find peak attention voxel in masked attention
    if attn_masked.max() > 0:
        peak = np.unravel_index(np.argmax(attn_masked), attn_masked.shape)
    else:
        peak = np.unravel_index(np.argmax(attn_vol), attn_vol.shape)
    d_peak, h_peak, w_peak = peak

    planes = [
        ('Axial',     ct_vol[d_peak],        attn_masked[d_peak]),
        ('Coronal',   ct_vol[:, h_peak, :],  attn_masked[:, h_peak, :]),
        ('Sagittal',  ct_vol[:, :, w_peak],  attn_masked[:, :, w_peak]),
    ]
